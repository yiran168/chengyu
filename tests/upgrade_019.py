"""Upgrade original 0.18/0.19/0.20 isolated databases using the current release code.
...
for version in ['7.4','8.5']:
    ...
    with tempfile.TemporaryDirectory(prefix='upgrade019-',dir=temp) as directory:
        ...
        assert phases[0]['schema'] in [10,11,12,13,14,15] and phases[1]['version']==json.loads((root/'RELEASE.json').read_text(encoding='utf-8'))['version']
