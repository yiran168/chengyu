...
$admin=$db->insert('cy_users',['username'=>'preview_admin','email'=>'preview@example.test','password_hash'=>password_hash(getenv('CY_ADMIN_PASSWORD'),PASSWORD_DEFAULT),'display_name'=>'站点管理员','role'=>'admin','status'=>'active','bio'=>'负责内容整理与网站维护。','created_at'=>time()]);
...
$a->catalog->save($admin,'collections',['name'=>'每周精选','description'=>'从阅读、灵感到日常创作，将值得留下的内容整理在一起。','content_ids'=>'1,2,3','active'=>1]);
$a->catalog->save($admin,'collections',['name'=>'常用工具','description'=>'好工具与真实交流，让想法向前多走一步。','content_ids'=>'7,8,9','active'=>1]);
$a->catalog->save($admin,'links',['name'=>'PHP','description'=>'查阅 PHP 官方文档与语言手册。','url'=>'https://www.php.net/','group_name'=>'开发资料','active'=>1]);
$a->catalog->save($admin,'announcements',['name'=>'站点公告','description'=>'欢迎交流，发帖前请阅读社区说明','audience'=>'public','route'=>'collections','active'=>1]);
...
$a->layout->save($admin,'articles',json_encode(['format'=>'chengyu-layout','version'=>1,'blocks'=>[
 ['id'=>'editorial_heading','type'=>'heading','title'=>'文章推荐','text'=>'教程、问题记录和使用经验。','tone'=>'plain','align'=>'left'],
 ['id'=>'selected_reading','type'=>'collection','title'=>'每周精选','collection'=>1,'size'=>3,'tone'=>'glass']
...
$poll=$a->content->save($author,['kind'=>'thread','title'=>$demo['poll_title'],'body'=>$demo['poll_body'],'excerpt'=>'欢迎投票，也可以在评论中补充建议。','status'=>'published','access_level'=>'public','comment_enabled'=>1,'thread_mode'=>'poll','poll_question'=>$demo['poll_question'],'poll_options'=>$demo['poll_options'],'poll_results'=>'after_vote'],true);
$question=$a->content->save($author,['kind'=>'thread','title'=>$demo['question_title'],'body'=>$demo['question_body'],'excerpt'=>'分类应该怎样划分？欢迎分享自己的做法。','status'=>'published','access_level'=>'public','comment_enabled'=>1,'thread_mode'=>'question'],true);
