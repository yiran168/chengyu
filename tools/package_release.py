...
EXCLUDE = {'__pycache__', '.git', '.pytest_cache', '.DS_Store'}
...
key = (ROOT / 'INSTALL_KEY.txt').read_text().strip()
digest = re.search(r"return '([a-f0-9]{64})'", (ROOT / 'site/install/key.php').read_text())
if not digest or digest.group(1) != sha(key.encode()):
    raise SystemExit('Private installation key does not match the public verifier.')
sitefiles = files(ROOT / 'site')
if any(key.encode() in p.read_bytes() for p in sitefiles):
    raise SystemExit('Private installation key leaked into an upload file.')

upload = OUT / ('chengyu-' + version + '-upload.zip')
complete = OUT / ('chengyu-' + version + '-complete.zip')
make_zip(upload, [(p, str(p.relative_to(ROOT / 'site'))) for p in sitefiles])
...
for p in files(ROOT):
    ...
    manifest.append(sha(p.read_bytes()) + '  ' + str(p.relative_to(ROOT)))
(ROOT / 'FILE_MANIFEST.sha256').write_text('\n'.join(manifest) + '\n')
make_zip(complete, [(p, 'chengyu/' + str(p.relative_to(ROOT))) for p in files(ROOT)])
...
with zipfile.ZipFile(upload) as uz, zipfile.ZipFile(complete) as cz:
    ...
    check('Private key excluded from upload and present in private complete package', 'INSTALL_KEY.txt' not in uz.namelist() and 'chengyu/INSTALL_KEY.txt' in cz.namelist())
...
for src, suffix in [('INSTALL_KEY.txt', 'INSTALL_KEY.txt'), ('docs/FEATURE_MATRIX.md', 'FEATURE_MATRIX.md'), ('docs/TEST_REPORT.md', 'TEST_REPORT.md'), ('docs/UPGRADE.md', 'UPGRADE.md')]:
