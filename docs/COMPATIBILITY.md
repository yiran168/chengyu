...
0.23.0 使用同一套 64 位 PHP 7.4–8.5 代码，功能不按小版本裁剪。需要原生 PDO MySQL 或 SQLite、OpenSSL、Fileinfo，以及可写的私有存储。默认不依赖 curl、mbstring、FFI、Composer、Node、Redis 或常驻服务。
...
Windows 安装网址路径已改用专用 WebPath，避免 dirname() 产生反斜杠 Cookie 路径；媒体签名探测有固定字节边界，避免 Windows Fileinfo 将大重复文本误判为二进制。兼容 7.4 不代表旧 PHP 运行时本身仍有官方安全维护。
