...
实际读取 Zibll 8.9 的 pages/archives.php：旧实现以 numberposts=-1 载入全部文章。读取 inc/functions/zib-comments-list.php 的置顶排除、置顶补充及分页处理，并读取 Shane/comments.php、inc/template-walker.php 的评论表单、Walker 与分页结构。只分析数据路径与交互需求，没有运行参考主题或复制其实现。

独立实现：澄屿复用 Content 的发布/分类可见性，按时间戳区间查询、每页 30 篇；评论使用有界查询、统一排序、每页 50 条以及服务端权限判断。源码审阅文件校验值保存在 evidence/020/reference-reviewed.json，标注实际阅读范围；哈希只证明文件定位，不能证明逐行审阅或法律上的无风险。

本轮网上复核 [子比官方更新记录](https://www.zibll.com/375.html)（V9.1，2026-09-01）及 [RiPro 官方版本记录](https://ritheme.com/theme/ripro-v5.html)（10.3，2026-08-18）。官方日志中的归档分页、评论限制、UI 动效与兼容性问题用作功能检查线索；不能据此说所有历史商业扩展已完成。子主题 Shane 不含 RiPro 父主题全部源码，审阅边界保持明确。
