...
$drivers=Database::drivers();$checks=[['64-bit PHP 7.4+',version_compare(PHP_VERSION,'7.4.0','>=')&&PHP_INT_SIZE>=8],['PDO MySQL or SQLite',(bool)array_intersect($drivers,['mysql','sqlite'])],['OpenSSL',extension_loaded('openssl')],['Built-in file type detection (no Fileinfo required)',class_exists(\Chengyu\Core\FileType::class)],['Writable app directory',is_writable($root.'/app')],['Writable storage directory',is_writable($root.'/storage')]];
...
$missingHelp=[];
$help=['PDO MySQL or SQLite'=>'Enable the PDO driver for the database selected in your hosting panel.','OpenSSL'=>'Ask your host to enable OpenSSL for the selected PHP version.','Built-in file type detection (no Fileinfo required)'=>'Upload the complete application files; the built-in file type detector is missing.','Writable app directory'=>'Allow PHP to write the app directory during installation.','Writable storage directory'=>'Allow PHP to write the storage directory.','Site-specific installation verifier'=>'Generate key.php using DEPLOY_PREPARE.html and upload it to install/key.php. Keep INSTALL_KEY.txt on your computer.'];
foreach($checks as [$label,$ok]){if(!$ok){$missingHelp[]=$help[$label]??$label;}}
