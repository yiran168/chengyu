...
{
    private App $a;private ?HttpClient $http;public function __construct(App $a,?HttpClient $http=null){$this->a=$a;$this->http=$http;}
    private function staff(array $u): void {if(!in_array($u['role'],['admin','editor'],true)||$u['status']!=='active'){throw new Problem('Staff access required.',403);}}
    ...
    private function owned(array $u,string $key): array {$this->staff($u);if(!preg_match('/^[a-f0-9]{48}$/D',$key)){throw new Problem('Object upload not found.',404);}$r=$this->a->db->one('SELECT * FROM cy_remote_uploads WHERE upload_key=? AND user_id=?',[$key,(int)$u['id']]);if(!$r){throw new Problem('Object upload not found.',404);}return $r;}
    private function checkHead(array $r,array $head,string $source=''): void
    {
        $h=$head['headers'];if((int)($h['content-length']??0)!==(int)$r['bytes']||($h['content-type']??'')!==$r['mime']||($h['x-amz-meta-cy-id']??'')!==$r['upload_key']||($source!==''&&($h['x-amz-meta-cy-source']??'')!==hash('sha256',$source))){throw new Problem('Stored object metadata does not match the manifest.',409);}
    }
    public function finish(array $u,string $key): int
    {
        $r=$this->owned($u,$key);if($r['state']==='complete'){return (int)$r['media_id'];}$lease=$this->a->db->transaction(function()use($r):array{$db=$this->a->db;$row=$db->one('SELECT * FROM cy_remote_uploads WHERE id=?'.$db->lock(),[(int)$r['id']]);if($row['state']==='complete'){return $row;}if((int)$row['expires_at']<time()||!in_array($row['state'],['pending','copying'],true)){throw new Problem('Object upload expired or closed.',409);}if((int)$row['lease_until']>time()){throw new Problem('Object verification is already running.',409);}$row['lease_until']=time()+180;$db->update('cy_remote_uploads',(int)$row['id'],['lease_until'=>$row['lease_until']]);return $row;});if($lease['state']==='complete'){return (int)$lease['media_id'];}$r=$lease;$snap=json_decode($this->a->crypto->open($r['config_cipher']),true,32,JSON_THROW_ON_ERROR);$s3=new S3($snap['provider'],$this->http);
        try{
            if($snap['etag']===''){
                ...
                // A slow verifier must not overwrite a newer worker's source snapshot or completion.
                $snap['etag']=$etag;
                $updated=$this->a->db->execute('UPDATE cy_remote_uploads SET state=?,config_cipher=? WHERE id=? AND lease_until=? AND state=?',[
                    'copying',$this->a->crypto->seal(json_encode($snap,JSON_THROW_ON_ERROR)),(int)$r['id'],(int)$r['lease_until'],'pending'
                ]);
                if($updated!==1){throw new Problem('Object verification lease changed. Retry the upload.',409);}
            }
            ...
            return $this->a->db->transaction(function()use($r,$snap,$u):int{$db=$this->a->db;$row=$db->one('SELECT * FROM cy_remote_uploads WHERE id=?'.$db->lock(),[(int)$r['id']]);if($row['state']==='complete'){return (int)$row['media_id'];}if((int)$row['lease_until']!==(int)$r['lease_until']){throw new Problem('Object verification lease changed. Retry the upload.',409);}$id=$db->insert('cy_media',['owner_id'=>(int)$u['id'],'name'=>$r['name'],'mime'=>$r['mime'],'bytes'=>(int)$r['bytes'],'file_key'=>'remote-'.$r['upload_key'],'is_private'=>1,'created_at'=>time(),'backend'=>'s3','remote_cipher'=>$this->a->crypto->seal(json_encode(['provider'=>$snap['provider'],'key'=>$snap['final']],JSON_THROW_ON_ERROR))]);$db->update('cy_remote_uploads',(int)$r['id'],['state'=>'complete','media_id'=>$id,'lease_until'=>0]);return $id;});
        }finally{$this->a->db->execute('UPDATE cy_remote_uploads SET lease_until=0 WHERE id=? AND lease_until=?',[(int)$r['id'],(int)$r['lease_until']]);}
    }
    public function download(array $media): string {$snap=json_decode($this->a->crypto->open($media['remote_cipher']),true,32,JSON_THROW_ON_ERROR);$inline=in_array($media['mime'],['video/mp4','audio/mpeg'],true);$disposition=($inline?'inline':'attachment').'; filename="file-'.(int)$media['id'].'"; filename*=UTF-8\'\''.rawurlencode($media['name']);return (new S3($snap['provider'],$this->http))->presign('GET',$snap['key'],120,[],['response-content-type'=>$media['mime'],'response-content-disposition'=>$disposition])['url'];}
}
