...
final class PlatformActions
{
    ...
    public function run(string $action,array $in,array $response): ?array
    {
        ...
        switch($action){
            case 'bulletin_save':$id=$a->bulletins->save($user,$in);$response['redirect']=$a->adminUrl('bulletins',['edit'=>$id]);break;
