...
$kind=\Chengyu\Core\Input::choice($_GET['kind']??'articles',['articles','bulletins']);
...
$items=$kind==='bulletins'?$a->bulletins->page(['size'=>30])['items']:$a->content->feed('article',null,['size'=>30])['items'];
foreach ($items as $item) {
    // Excerpts only. Never serialize protected_body into a public feed.
    if($kind==='bulletins'){$item['excerpt']=\Chengyu\Core\Input::clip($item['body'],400);$item['created_at']=$item['publish_at'];}
    $url = $a->config['url'] . substr($kind==='bulletins'?$a->url('bulletin',['id'=>$item['id']]):item_url($item), strlen($a->basePath));
